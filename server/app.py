...
@app.route('/baked_goods/<int:paramid>', methods = ['GET', 'DELETE'])
def baked_goods_by_id(paramid):
    bakery_good = BakedGood.query.filter(BakedGood.id == paramid).first()

    if request.method == 'GET':
        bakery_obj = bakery_good.to_dict()
        response = make_response(bakery_obj, 200)
        return response
    elif request.method == 'DELETE':
